...
class MazeDraw:
    ...
    def __init__(self, graph: LatticeGraph, origin: int, goal: int) -> None:
        ...
        if screen_rows < self._rows or screen_cols < self._cols:
            print(f"Your console window is too small to display the maze. Please, resize it to at least {self._rows} rows and {self._cols} columns.")
            self.bail_out()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--greedy",
        action="store_true",
        help="Run greedy search"
    )
    parser.add_argument(
        "--maximum",
        action="store_true",
        help="Run maximum traversal search"
    )
    parser.add_argument(
        "--viz",
        action="store_true",
    ...
    if exclusion != 1:
        print ("Error: Program expects one type of traversal. Please \
